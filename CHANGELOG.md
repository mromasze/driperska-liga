...
- **Po dolnej krawędzi boksu z opiniami jedzie złote światełko odliczające do zmiany cytatu.** Bez niego zmiana co trzy sekundy wygląda na przypadkową — nie wiesz, czy masz jeszcze moment na doczytanie. Pasek przejeżdża dokładnie jedną rotację, przy zatrzymaniu kursorem **zamiera w miejscu** zamiast wracać na start (rotacja zatrzymuje się w tym samym momencie), a strzałki restartują go razem z odliczaniem. Animacja idzie po `transform`, więc kompozytuje się na GPU; przy `prefers-reduced-motion` karuzela i tak nie rotuje, więc światełka po prostu nie ma.
