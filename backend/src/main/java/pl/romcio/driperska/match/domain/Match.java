...
public class Match {
    ...
    private DrawMode drawMode = DrawMode.BALANCED;
    ...
    /** Moves to {@code target}, enforcing the lifecycle rules. */
    public void transitionTo(MatchStatus target) {
        if (!status.canTransitionTo(target)) {
            throw new InvalidTransitionException(
                    "Niedozwolone przejście meczu: %s → %s".formatted(status, target));
        }
        this.status = target;
    }
    ...
    public List<UUID> getPoolPlayerIds() {
        return poolPlayerIds;
    }

    public void setPoolPlayerIds(List<UUID> poolPlayerIds) {
        this.poolPlayerIds = new ArrayList<>(poolPlayerIds);
    }
}
